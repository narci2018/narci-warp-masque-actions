...
var (
	...
	webGroup = flagGroup{"Web server", []flagSpec{
		{"l", "listen", "ADDR:PORT", "address and port the WebUI server listens on (default 0.0.0.0:29880)"},
		{"P", "socks-port", "PORT", "local SOCKS5 proxy port when activated from WebUI (default 29881)"},
		{"a", "account", "FILE", "cached WARP account file (default warpscout-account.json)"},
	}}
	...
	pagesGroup = flagGroup{"Cloudflare Pages static export", []flagSpec{
		{"o", "out-dir", "DIR", "directory for static pages output (default public)"},
		{"", "auto-register", "", "automatically register a WARP account if none exists (default true)"},
	}}
)
...
func setupWebFlags(fs *flag.FlagSet, o *options) {
	strFlag(fs, &o.listen, "0.0.0.0:29880", "l", "listen")
	strFlag(fs, &o.accountPath, defaultAccount, "a", "account")
	intFlag(fs, &o.port, 29881, "P", "socks-port")
	strFlag(fs, &o.proxy, "", "x", "proxy")
}

func setupPagesFlags(fs *flag.FlagSet, o *options) {
	addNetFlags(fs, o)
	addAWGFlags(fs, o)
	intFlag(fs, &o.tunnelParallel, 16, "jt", "tunnel-jobs")
	intFlagValidate(fs, &o.perSubnet, 10, "n", "sample", validateSample)
	fs.IntVar(&o.port, "port", 0, "")
	fs.StringVar(&o.sweepPorts, "sweep-ports", "", "")
	fs.StringVar(&o.pingTarget, "ping-target", "", "")
	strFlag(fs, &o.proto, protoAWG, "p", "proto")
	strFlag(fs, &o.outDir, "public", "o", "out-dir")
	boolFlag(fs, &o.tunPingCheck, "P", "tun-ping")
	fs.IntVar(&o.tunPingCount, "tun-ping-count", 0, "")
	fs.BoolVar(&o.speed, "speed", false, "")
	boolFlag(fs, &o.full, "f", "full")
